...
\begin{frame}
  ...
  Los sub\'indices de cada elemento $a_{i,j}$ denotan la posici\'on del mismo:
...
\begin{frame}
  Podemos extraer renglones y columnas de la matrix \eqref{A}: El $i-$es\'imo
  rengl\'on es
  $$
    R_{i}=
    \begin{pmatrix}
...
\begin{frame}
  ...
  En el caso de los vectores renglones y columnas, podemos omitir el sub\'indice
  fijo
...
\begin{frame}
  En matem\'aticas, la \emph{teor\'ia de grafos} estudia estructuras
...
\begin{frame}
  \begin{rem}
    ...
    puede confundir con el concepto de teor\'ia de conjuntos. En este material, a
...
\begin{frame}
  Eliminando aristas innecesarias, no es dif\'icil ver que cualquier camino de
...
\begin{frame}{Conexidad y componentes conexas}
  ...
  \pause Por ejemplo, el grafo \ref{fig:md0504}(a) es conexo, pero no as\'i el
