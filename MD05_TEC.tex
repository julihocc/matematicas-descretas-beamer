...
\begin{frame}
  \begin{rem}
    ...
    que la suma, resta y multiplicaci\'on entre entre elementos de $F$ también
...
\begin{frame}
  \begin{defn}
    ...
    \begin{equation}
      ...
      \end{pmatrix}
    \end{equation}
    donde $R_{i}$ es el $i-$ésimo rengl\'on de $A$ y $C_{k}$ es la $k-$ésima
    columna de $B.$
  \end{defn}
  ...
  \begin{rem}
    \begin{itemize}
      \item Para que esta multiplicaci\'on tenga sentido, los renglones de $A$ y
            ...
            \pause
      \item La matriz resultante tendrá dimensi\'on $m \times l.$ \pause
...
\begin{frame}
  \begin{rem}
    Debido a una ambig\"uedad en la traducci\'on del inglés al espa\~nol, en
...
\begin{frame}{Gráfos finitos y triviales}
  ...
  vértices y un n\'umero finito de aristas. \pause

  Observe que un n\'umero finito de vértices implica un n\'umero finito de
  aristas; pero no lo contrario no es necesariamente cierto. \pause

  Diremos que un grafo con un \'unico vértice, sin aristas,  es
  \emph{trivial.}\
\end{frame}
...
\begin{frame}
  \begin{rem}
    ...
    indicando s\'olo los vértices que recorre:
    $$v_{0}, v_{1},..., v_{n}.$$
  \end{rem}
\end{frame}
...
\begin{frame}
  ...
  corto, es decir, con el meno n\'umero de aristas. ?`Cuál es el camino más
  corto?
\end{frame}
...
\begin{frame}
  \begin{rem}
    ...
    \end{center}
    es una relaci\'on de equivalencia en el conjunto de vértices del grafo $G,$
...
\begin{frame}
  Si las aristas o los vértices de un digrafo están etiquetas con alg\'un
...
\begin{frame}
  \begin{rem}
    Si definimos
    $$B_{r}= \sum_{i=1}^{r}A^{i},$$
    entonces la entrada $i,j$ de esta matriz nos indicará el n\'umero de caminos
